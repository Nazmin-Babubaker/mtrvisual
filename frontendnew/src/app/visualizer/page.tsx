...
// ── Mini Globe for the toggle view ───────────────────────────────────────────
function GlobeEarth({ hops }: { hops: any[] }) {
  const meshRef = useRef<THREE.Mesh>(null!);
  const cloudsRef = useRef<THREE.Mesh>(null!);

  const [dayTex, cloudTex] = [
    new THREE.TextureLoader().load("https://threejs.org/examples/textures/planets/earth_atmos_2048.jpg"),
    new THREE.TextureLoader().load("https://threejs.org/examples/textures/planets/earth_clouds_1024.png"),
  ];

  useFrame((_, delta) => {
    meshRef.current.rotation.y += delta * 0.06;
    cloudsRef.current.rotation.y += delta * 0.072;
  });

  // Plot hop markers as colored dots on the sphere surface
  const hopMarkers = hops
    .filter((h) => h.geo?.lat && h.geo?.lng)
    .map((h, i) => {
      const type = classifyHop(h, i, hops.filter((x) => x.geo?.lat).length);
      const cfg = TYPE_CONFIG[type];
      const lat = (h.geo.lat * Math.PI) / 180;
      const lng = (h.geo.lng * Math.PI) / 180;
      const r = 1.52;
      return {
  ...
  return (
    <>
      ...
      <mesh ref={cloudsRef}>
        ...
        <meshPhongMaterial map={cloudTex} transparent opacity={0.32} depthWrite={false} />
      </mesh>
      <mesh>
...
function GlobeView({ hops }: { hops: any[] }) {
  return (
    <div style={{ width: "100%", height: "100%", background: "#050c18" }}>
      <Canvas camera={{ position: [0, 0, 4], fov: 40 }} gl={{ antialias: true, alpha: false }}>
...
function HopItem({ hop, index, total, selected, onSelect }: any) {
  ...
  const rttColor = rtt === null ? "rgba(255,255,255,0.25)"
    : rtt < 50 ? "#22c55e" : rtt < 150 ? "#f59e0b" : "#ef4444";
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .search-btn {
          ...
          bottom: 4px;
          padding: 0 16px;
          background: ${loading ? "rgba(59,130,246,0.15)" : "rgba(59,130,246,0.2)"};
        ...
        .packet-dot {
          ...
          border-radius: 50%;
          background: #63c8ff;
          box-shadow: 0 0 8px #63c8ffaa;
          animation: packetGlow 1.2s ease-in-out infinite;
        }
        @keyframes packetGlow {
          0%,100% { box-shadow: 0 0 6px #63c8ff88; }
          50%      { box-shadow: 0 0 14px #63c8ff; }
        }
      `}</style>
